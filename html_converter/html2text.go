...
//export FromString_Py
func FromString_Py(input *C.char) (*C.char, *C.char) {
	option := Options{OmitLinks: false, PrettyTables: true}
	text, err := FromString(C.GoString(input), option)
	if err != nil {
		return C.CString(""), C.CString(fmt.Sprintf("%s", err))
	}

	return C.CString(text), C.CString("")
}
...
func (ctx *textifyTraverseContext) handleElement(node *html.Node) error {
	...
	case atom.H1, atom.H2, atom.H3, atom.H4, atom.H5:
		...
		headlinePrefix := "#"
		divider := strings.Repeat(headlinePrefix, dividerLen)
